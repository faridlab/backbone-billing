...
This directory contains SQLx database migrations for the billing module. These migrations create a comprehensive database schema with support for:
...
### 001_create_billing_table.sql
Creates the main `billing` table with:
...
### 002_create_billing_events_table.sql
Creates the event store table `billing_events` with:
...
### 003_create_billing_audit_table.sql
Creates the audit trail table `billing_audit` with:
...
### 004_create_billing_indexes_and_triggers.sql
...
### 005_create_billing_views.sql
Creates optimized database views:
- `billing_active` - Only active entities
- `billing_stats` - Materialized statistics view
- `billing_with_activity` - Entities with recent activity
- `billing_audit_summary` - Audit summary per entity
- `billing_event_timeline` - Chronological event view
- `billing_audit_timeline` - Chronological audit view
- `billing_by_metadata` - Entities with extracted metadata
- `billing_performance_metrics` - Daily performance metrics
...
billing
├── Main Table: billing
├── Event Store: billing_events
├── Audit Trail: billing_audit
├── Views (Optimized):
│   ├── billing_active
│   ├── billing_stats (materialized)
│   ├── billing_with_activity
│   ├── billing_audit_summary
│   ├── billing_event_timeline
│   ├── billing_audit_timeline
│   ├── billing_by_metadata
│   └── billing_performance_metrics (materialized)
└── Functions:
    ├── search_billing()
    ├── get_billing_statistics()
    ├── refresh_billing_materialized_views()
    └── schedule_billing_refresh()
```
...
CREATE POLICY billing_read_policy ON billing
...
FROM pg_matviews
WHERE matviewname LIKE 'billing_%';
