...
metaphor migration generate Invoice billing
metaphor migration run
...
(`billing` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed billing          # run Rust seeders
metaphor migration generate-seeds billing  # emit SQL seed files
```
