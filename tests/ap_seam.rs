...
async fn purchase_invoice_bills_po_across_three_modules() {
    ...
    let gl = GlAdapter {
        svc: PostingService::new(Arc::new(
        ...
        .create_purchase_invoice(NewPurchaseInvoice {
            ...
            tax_lines: vec![
                NewTaxLine {
                    account_id: coa["1520"],
                    basis: "input".into(),
                    description: Some("PPN Masukan 11%".into()),
                    rate: d("11"),
                    tax_amount: d("99000"),
                ...
                NewTaxLine {
                    ...
                    description: Some("PPh 23 2%".into()),
                    rate: d("2"),
                    tax_amount: d("18000"),
                    taxable_base: Decimal::ZERO,
                    tax_template_id: None,
    ...
    assert_eq!(
        journal_totals(&pool, out.journal_id).await,
        (d("999000"), d("999000"))
    );
    ...
    let posted = evts
        .iter()
        .find_map(|e| match e {
            BillingEvent::PurchaseInvoicePosted(p) if p.source_po_id == Some(po) => Some(p.clone()),
            _ => None,
        })
        .expect("PurchaseInvoicePosted for our PO");
    assert_eq!(posted.grand_total, d("981000.00"));
    let billed: Vec<(Uuid, Decimal)> = posted
        .billed_lines
        .iter()
    ...
    let (ps, jid): (String, Option<Uuid>) = sqlx::query_as(
        "SELECT posting_state::text, journal_id FROM billing.purchase_invoices WHERE id=$1",
    )
    .bind(inv)
    .fetch_one(&pool)
    .await
