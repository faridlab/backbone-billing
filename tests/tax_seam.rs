...
//!
//! Lives HERE (billing) rather than in backbone-tax: the cargo edge runs billing → tax (the
//! document-engine dep), so the tax crate must never depend back on billing. Requires
//! DATABASE_URL (:5433) with BOTH the `billing` and `tax` schemas migrated (each migration
//! `CREATE SCHEMA IF NOT EXISTS`; migrate externally, like the other seam tests).
...
async fn sales_post_routes_to_tax_and_assigns_gapless_efaktur() {
    ...
    assert_eq!(
        posted.taxable_base,
        d("1000000.00"),
        "enriched event carries the net base"
    );
    assert_eq!(
        posted.output_total,
        d("110000.00"),
        "enriched event carries PPN output"
    );
    ...
    let number: String =
        sqlx::query_scalar("SELECT number FROM tax.efaktur_documents WHERE tax_transaction_id=$1")
            .bind(txn)
            .fetch_one(&pool)
            .await
            .unwrap();
...
async fn purchase_post_routes_to_tax_without_efaktur() {
    ...
    let posted = rec
        ...
        .iter()
        .find_map(|e| match e {
            BillingEvent::PurchaseInvoicePosted(p) if p.invoice_id == inv => Some(p.clone()),
            _ => None,
        })
    ...
    let tax = TaxModule::builder()
        ...
        .build()
        .unwrap();
    let (_txn, efaktur) = tax
        .efaktur_service
        .record_tax_transaction(&purchase_to_posted_for_tax(&posted))
        .await
        .unwrap();
    ...
    assert_eq!(
        txn_count, 1,
        "exactly one TaxTransaction for this purchase invoice"
    );
}
