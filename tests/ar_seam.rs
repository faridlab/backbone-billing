...
async fn sales_invoice_posts_balanced_revenue_into_the_real_gl() {
    ...
    let inv = billing.create_sales_invoice(NewSalesInvoice {
        ...
        lines: vec![NewInvoiceLine { item_id: item, account_id: coa["4000"], description: None,
            quantity: d("10"), unit_price: d("100000") }],
        tax_lines: vec![NewTaxLine { account_id: coa["2200"], basis: "output".into(),
            description: Some("PPN Keluaran 11%".into()), rate: d("11"), tax_amount: d("110000") }],
    }).await.unwrap();
    ...
    let jrow = sqlx::query("SELECT total_debit, total_credit, line_count FROM accounting.journals WHERE id=$1")
        .bind(out.journal_id).fetch_one(&pool).await.unwrap();
    ...
    let rev_credit: Decimal = sqlx::query_scalar(
        ...
        .bind(out.journal_id).bind(coa["4000"]).fetch_one(&pool).await.unwrap();
    assert_eq!(rev_credit, d("1000000"));
    let ppn_credit: Decimal = sqlx::query_scalar(
        "SELECT credit_amount FROM accounting.journal_lines WHERE journal_id=$1 AND account_id=$2")
        .bind(out.journal_id).bind(coa["2200"]).fetch_one(&pool).await.unwrap();
...
async fn concurrent_double_post_yields_one_journal() {
    ...
    let inv = billing.create_sales_invoice(NewSalesInvoice {
        ...
        source_so_id: None, posting_date: day(), due_date: None, currency: None,
        receivable_account_id: coa["1200"],
        lines: vec![NewInvoiceLine { item_id: Uuid::new_v4(), account_id: coa["4000"], description: None,
    ...
    let p1 = pool.clone();
    let first = {
        let gl = GlAdapter { svc: PostingService::new(Arc::new(SqlxPostingRepository::new(p1.clone()))) };
