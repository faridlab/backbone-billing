//! Domain policy for PaymentTermLine aggregate
//!
...
/// Domain policy for PaymentTermLine — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PaymentTermLine>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PaymentTermLineDomainPolicy = PermitAllPolicy<PaymentTermLine>;
