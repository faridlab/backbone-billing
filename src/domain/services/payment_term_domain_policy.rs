//! Domain policy for PaymentTerm aggregate
//!
...
/// Domain policy for PaymentTerm — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PaymentTerm>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PaymentTermDomainPolicy = PermitAllPolicy<PaymentTerm>;
