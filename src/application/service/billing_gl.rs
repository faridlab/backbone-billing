//! Outbound GL-posting port (hand-authored, user-owned) — billing side of the GL-posting contract.
//!
//! Billing is the AR/AP emitter: a Sales Invoice posts `Dr A/R · Cr Revenue · Cr PPN Output`; a
//! Purchase Invoice posts `Dr Expense · Dr PPN Input · Cr A/P · Cr PPh`. It emits a serialized
//! `AccountingPostEnvelope` reached only through a `GlPostSink`; the ACL adapter maps it into
//! accounting's `PostingRequest`. Zero normal Cargo edge (the envelope is the wire contract). Same
//! shape as the selling/inventory/buying ports — the contract is duplicated per producer by design.
...
pub struct AccountingPostEnvelope {
    ...
    /// Posting source discriminator — "order" (sales invoice) | "expense" (purchase invoice).
