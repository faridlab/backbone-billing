...
impl BillingWriteService {
    ...
    pub async fn build_ar_post(
        ...
        for t in &tax_rows {
            lines.push(
                GlPostLine::credit(t.account_id, t.tax_amount).with_description("PPN Output"),
            );
        }
        ...
        let env = AccountingPostEnvelope {
            ...
            description: Some("Sales invoice".into()),
            lines,
        };
        if !env.is_balanced() {
    ...
    pub async fn post_sales_invoice(
        ...
        if let Some(o) = self.short_circuit_sales_posted(invoice_id).await? {
            return Ok(o);
        }
    ...
    pub async fn reverse_sales_invoice(
        &self,
        invoice_id: Uuid,
        sink: &dyn GlPostSink,
    ) -> Result<PostOutcome, BillingError> {
        ...
        let lines = fwd
            ...
            .collect();
        let env = AccountingPostEnvelope {
            idempotency_key: format!("reversal:{invoice_id}"),
            posting_type: "reversal".into(),
            reverses_post_id: orig_post,
            lines,
        ...
        match sink.post(&env).await {
            ...
            Err(rej) => Err(BillingError::GlRejected {
                code: rej.code,
                message: rej.message,
            }),
        }
    ...
    pub(super) async fn short_circuit_sales_posted(
        &self,
        invoice_id: Uuid,
